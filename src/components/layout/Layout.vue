<template>
    <div id="layout">
        ...
        <div v-if="!locked" class="uk-margin">
            <!-- Save Layout -->
            <button @click="saveLayout()" class="uk-button button-primary">Save Layout</button>
            <!-- Name input -->
            <input v-model="layout.name" type="text" class="uk-input uk-form-width-medium button-margin" placeholder="Layout Name">
            <!-- This component contains the button and modal to load a layout -->
            <LoadLayout @loadedLayout="layout = $event"></LoadLayout>
            <!-- Lock -->
            <button @click="lock(true)" class="uk-button button-margin button-secondary">Lock</button>
            <button @click="resetLayout()" class="uk-button button-margin">Reset</button>
            <!-- Delete -->
            <button v-if="layout.id != null" @click="deleteLayout()" class="uk-button button-margin button-danger uk-float-right">Delete Layout</button>
        </div>
        <div v-else class="uk-margin">
            <button @click="lock(false)" class="uk-button button-margin button-secondary">Unlock</button>
        </div>
        ...
        <ul class="uk-grid-small" uk-sortable="handle: .uk-card" uk-grid="masonry: false">
            ...
            <ModuleSlot v-if="!locked" :id="0" :width="'1/3'" @add="addModule()" :content="{'driver': 'add'}"></ModuleSlot>
        </ul>
    ...
    methods: {
        ...
        addModule() {
            ...
            if (mods.length == 0) {
                ...
                mods.push({id: latest + 1, width: "1/3", content: {driver: "none"}});
            }
        },
        ...
        lock(status) {
            this.locked = status;
            if (status) {
                this.layout.modules = this.layout.modules.filter(mdl => mdl.content.driver != 'none');
            }
        },
